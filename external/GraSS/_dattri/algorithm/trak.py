...
class TRAKAttributor(BaseAttributor):
    ...
    def __init__(
        ...
        track_projection_time: bool = False,
    ) -> None:
        ...
        self.track_projection_time = track_projection_time
        self.projection_only_time_s = 0.0
        self.projection_build_time_s = 0.0
        self.projection_only_batches = 0
        self.projection_only_time_s_list = []
        self.projection_build_time_s_list = []

    def reset_projection_timer(self) -> None:
        """Reset projection-only timing counters."""
        self.projection_only_time_s = 0.0
        self.projection_build_time_s = 0.0
        self.projection_only_batches = 0
        self.projection_only_time_s_list = []
    ...
    def cache(
        ...
        if self.track_projection_time:
            self.reset_projection_timer()
        inv_XTX_XT_list = []
        running_Q = 0
        running_count = 0
        use_cuda_timing = (
            self.track_projection_time
        ...
        for ckpt_idx in range(len(self.task.get_checkpoints())):
            ...
            for train_data in tqdm(
                ...
                if self.track_projection_time:
                    ...
                    projector_fn = random_project(
                        grad_t,
                        batch_size,
                        **self.projector_kwargs,
                    )
                    build_elapsed = time.perf_counter() - build_start
                    self.projection_build_time_s += build_elapsed
                    self.projection_build_time_s_list.append(build_elapsed)
                    ...
                    if use_cuda_timing:
                        ...
                        start_event.record()
                        grad_p = (
                            projector_fn(grad_t, ensemble_id=ckpt_idx)
                            .clone()
                            .detach()
                        )
                        end_event.record()
                        ...
                        self.projection_only_time_s += elapsed_s
                        self.projection_only_time_s_list.append(elapsed_s)
                    else:
                        ...
                        self.projection_only_time_s += elapsed_s
                        self.projection_only_time_s_list.append(elapsed_s)
                    self.projection_only_batches += 1
                else:
                    grad_p = (
                        random_project(
                            grad_t,
