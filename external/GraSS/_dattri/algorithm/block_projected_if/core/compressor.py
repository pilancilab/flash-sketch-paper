...
from _dattri.func.projection import AbstractProjector, random_project
...
def _setup_linear_projector(  # noqa: PLR0914 - Complex projector setup requires many local variables for dimension calculations
    ...
    projector_func = random_project(
        ...
        proj_type=kwargs.get("proj_type", "normal"),
        device=kwargs.get("device", "cpu"),
        proj_kappa=kwargs.get("proj_kappa", 2),
        proj_s=kwargs.get("proj_s", 2),
        proj_block_rows=kwargs.get("proj_block_rows", 128),
    )
...
def setup_compression_kwargs(
    args,
    device: torch.device,
) -> Tuple[Optional[Dict[str, Any]], Optional[Dict[str, Any]]]:
    """Setup compression kwargs for dattri's BlockProjectedIFAttributor.

    Compression pipeline:
    - Sparsification (Stage 1): ALWAYS factorized, format TYPE-DIM*DIM
      Each component is projected to DIM, so total dimension is DIM*DIM after Kronecker product.
    - Projection (Stage 2): ALWAYS non-factorized, format TYPE-DIM
      Projects the flattened Kronecker product to DIM.

    Projection Types:
    - normal, rademacher, sjlt, fjlt, random_mask, grass, selective_grass, identity: Standard projection types
    - selective_mask: Auto-loads pre-computed indices. Supports two formats:
      - Factorized (for sparsification): ./SelectiveMask/mask_{dim}*{dim}/{layer_name}.pt
      - Non-factorized (for projection): ./SelectiveMask/mask_{dim}/result.pt
    - selective_grass: Combines selective_mask with SJLT. Same mask path conventions as selective_mask,
      with intermediate_dim = N * proj_dim (N defaults to 4, or specify as selective_grass_N)

    Args:
        args: Argument namespace with sparsification and projection settings.
            Expected attributes:
              - sparsification: str like "random_mask-128*128" (factorized)
              - projection: str like "sjlt-4096" or "selective_mask-4096" (non-factorized)
              - seed: int for reproducibility
        device: Device to run computations on

    Returns:
        Tuple of (sparsifier_kwargs, projector_kwargs) for dattri

    Example:
        >>> args.sparsification = "random_mask-128*128"
        >>> args.projection = "sjlt-4096"
        >>> sparsifier_kwargs, projector_kwargs = setup_compression_kwargs(args, device)

        # With SelectiveMask for sparsification (factorized, loads from ./SelectiveMask/mask_128*128/):
        >>> args.sparsification = "selective_mask-128*128"
        >>> sparsifier_kwargs, projector_kwargs = setup_compression_kwargs(args, device)

        # With SelectiveMask for projection (non-factorized, loads from ./SelectiveMask/mask_4096/result.pt):
        >>> args.projection = "selective_mask-4096"
        >>> sparsifier_kwargs, projector_kwargs = setup_compression_kwargs(args, device)
    """
    ...
    # Parse sparsification settings (Stage 1 - ALWAYS factorized)
    if hasattr(args, 'sparsification') and args.sparsification is not None:
        # Handle "identity" as special case (no sparsification)
        if args.sparsification == "identity":
            sparsifier_kwargs = {
                "proj_dim": -1,
            ...
            proj_type, dim_str = args.sparsification.split("-")
            # Sparsification is ALWAYS factorized, requires DIM*DIM format
            if "*" not in dim_str:
                raise ValueError(
                    f"Sparsification is factorized and requires DIM*DIM format. "
                    f"Got '{args.sparsification}'. Use format 'TYPE-DIM*DIM' (e.g., 'random_mask-128*128')."
                )
            dims = dim_str.split("*")
            assert dims[0] == dims[1], \
                "Sparsification dimensions must be equal (factorized projection uses same dim for both components)."
            proj_dim = int(dims[0])
    ...
    # Parse projection settings (Stage 2 - ALWAYS non-factorized)
    if hasattr(args, 'projection') and args.projection is not None:
        # Handle "identity" as special case (no projection)
        if args.projection == "identity":
            ...
            proj_type, dim_str = args.projection.split("-")
            # Projection is ALWAYS non-factorized, expects single DIM format
            if "*" in dim_str:
                raise ValueError(
                    f"Projection is non-factorized and does not support DIM*DIM format. "
                    f"Got '{args.projection}'. Use format 'TYPE-DIM' (e.g., 'sjlt-4096')."
                )
            proj_dim = int(dim_str)
            ...
            if proj_type in (
                ...
                projector_kwargs["proj_kappa"] = int(getattr(args, "flashsketch_kappa", 2))
                projector_kwargs["proj_s"] = int(getattr(args, "flashsketch_s", 2))
                projector_kwargs["proj_block_rows"] = int(
                    getattr(args, "flashsketch_block_rows", 128)
                )
                flash_seed = getattr(args, "flashsketch_seed", None)
                if flash_seed is not None:
                    projector_kwargs["proj_seed"] = int(flash_seed)
    else:
        # Default to identity projection (no projection)
        projector_kwargs = {
            "proj_dim": -1,
            "proj_max_batch_size": 64,
            "proj_seed": seed,
