...
class MLPMnist(nn.Module):
    ...
    def __init__(self, dropout_rate: float = 0.1, activation: str = "relu") -> None:
...
def create_mlp_model(dataset: str, **kwargs) -> nn.Module:
    ...
    """
    if dataset == "mnist":
        activation = os.environ.get("GRASS_MLP_ACTIVATION")
        dropout = os.environ.get("GRASS_MLP_DROPOUT_RATE")
        if activation is not None:
            kwargs["activation"] = activation
        if dropout is not None:
            kwargs["dropout_rate"] = float(dropout)
